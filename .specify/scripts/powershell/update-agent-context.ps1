...
.PARAMETER AgentType
Optional agent key to update a single agent. If omitted, updates all existing agent files and leaves the workspace unchanged when none exist.
...
function Parse-PlanData {
    ...
    $script:NEW_FRAMEWORK   = Extract-PlanField -FieldPattern 'Framework' -PlanFile $PlanFile
    if (-not $script:NEW_FRAMEWORK) {
        $script:NEW_FRAMEWORK = Extract-PlanField -FieldPattern 'Primary Dependencies' -PlanFile $PlanFile
    }
    $script:NEW_DB          = Extract-PlanField -FieldPattern 'Storage' -PlanFile $PlanFile
    $script:NEW_PROJECT_TYPE = Extract-PlanField -FieldPattern 'Project Type' -PlanFile $PlanFile
    if (-not $script:NEW_PROJECT_TYPE) {
        $script:NEW_PROJECT_TYPE = 'SEI module'
    }
...
function Get-ProjectStructure { 
    ...
    if ($ProjectType -match 'SEI module') {
        return "fontes/sei/src/main/php/sei/web/modulos/<inst>/<modulo>/`nfontes/sei/src/main/php/sei/scripts/`nfontes/sei/src/main/php/sip/scripts/`nspecs/"
    } elseif ($ProjectType -match 'web') {
...
function Get-CommandsForLanguage { 
    ...
    switch -Regex ($Lang) {
        ...
        'PHP' { return "php -l <changed-file.php>" }
...
function Get-LanguageConventions { 
    ...
    if ($Lang -match 'PHP') {
        'PHP: follow SEI/InfraPHP conventions; save changed PHP files as ISO-8859-1 (Latin-1)'
    } elseif ($Lang) {
...
function Update-AllExistingAgents {
    ...
    if (-not $found) {
        # Keep shared Spec Kit artifacts untouched when no agent context file exists.
        Write-Info 'No existing agent context files found; pass an agent type to create one.'
    }
    return $ok
}
