...
if ($Help) {
    Write-Host "Usage: ./create-new-feature.ps1 [-Json] [-DryRun] [-AllowExistingBranch] [-NoBranch] [-ShortName <name>] [-Number N] [-Timestamp] <feature description>"
    Write-Host ""
    Write-Host "Options:"
    Write-Host "  -Json               Output in JSON format"
    Write-Host "  -DryRun             Compute branch name and paths without creating branches, directories, or files"
    Write-Host "  -AllowExistingBranch  Switch to branch if it already exists instead of failing"
    Write-Host "  -NoBranch           Do not create or switch to a branch (for manual branch management)"
...
if (-not $FeatureDescription -or $FeatureDescription.Count -eq 0) {
    Write-Error "Usage: ./create-new-feature.ps1 [-Json] [-DryRun] [-AllowExistingBranch] [-NoBranch] [-ShortName <name>] [-Number N] [-Timestamp] <feature description>"
    exit 1
}
...
if (-not $DryRun) {
    if ($NoBranch) {
        Write-Warning "[specify] Skipping branch creation (-NoBranch specified)"
    } elseif ($hasGit) {
        $branchCreated = $false
        try {
