...
#    - Leaves the workspace unchanged if no agent context file exists
#
...
parse_plan_data() {
    ...
    NEW_FRAMEWORK=$(extract_plan_field "Framework" "$plan_file")
...
get_project_structure() {
    ...
    if [[ "$project_type" == *"SEI module"* ]]; then
        echo "fontes/sei/src/main/php/sei/web/modulos/<inst>/<modulo>/\\nfontes/sei/src/main/php/sei/scripts/\\nfontes/sei/src/main/php/sip/scripts/\\nspecs/"
...
get_language_conventions() {
    local lang="$1"
    if [[ "$lang" == *"PHP"* ]]; then
        echo "PHP: follow SEI/InfraPHP conventions; save changed PHP files as ISO-8859-1 (Latin-1)"
    else
...
update_all_existing_agents() {
    ...
    # If no agent files exist, keep shared Spec Kit artifacts untouched.
    if [[ "$_found_agent" == false ]]; then
        log_info "No existing agent context files found; pass an agent type to create one."
    fi
