...
9. Revisoes nao avaliam spec nesta versao. `sei-revisao-tecnica` cobre os standards SEI; `code-review` apenas fixa o delta commitado e delega Standards para ela.
10. `code-review` exige um ponto fixo Git para revisar o delta commitado. Worktree sem commit deve usar `sei-revisao-tecnica` diretamente ou fornecer um diff para essa revisao tecnica.
...
| Revisao tecnica / security / compliance SEI | revisao tecnica, security review, compliance, gates, permissoes, transacoes, risco operacional, passivo tecnico ou modulo SEI completo | `sei-revisao-tecnica` | `sei-verificacao-pagina`, `sei-verificacao-rn`, `sei-verificacao-banco-dados`, `sei-verificacao-controladores`, `sei-verificacao-tarefa`, `sei-guardrails-modulo`, `sei-testes-validacao` (conforme artefatos no escopo) | diff fornecido, PR ou refs acessiveis, arquivos, dimensao tecnica ou modulo; spec nao e necessaria | entrada tecnica indeterminada ou gate aplicavel sem evidencia suficiente para execucao |
...
| Code review de delta commitado | code review de mudancas commitadas desde commit, branch ou tag; PR com base fixa; standards do repositorio | `code-review` | `sei-revisao-tecnica` em Standards | ponto fixo Git e `HEAD` commitado; spec nao e avaliada nesta versao | ponto fixo ausente ou invalido, merge-base invalido ou delta commitado vazio |
