...
- Todas as frases e todos os componentes da fórmula selecionada são obrigatórios. Se algum termo permanecer indeterminado depois da busca, **publique a descrição assim mesmo**: preencha os termos comprovados e marque cada termo aberto com o marcador de lacuna definido em `formato-dicionario-de-dados.md`, seção "Marcação de lacuna". A frase da fórmula é preservada inteira; o marcador ocupa apenas o termo aberto.
...
Os três itens mostram a forma da enumeração, não uma cardinalidade fixa. Na descrição publicada, inclua um item para cada valor do domínio completo e comprovado, mesmo quando o domínio tiver um único valor comprovado; domínio incompleto bloqueia a descrição. No termo `[quantidade]`, informe apenas a quantidade de valores comprovados: a enumeração vai somente na lista final. Mantenha cada valor como um item independente da lista, mesmo quando vários compartilharem alguma característica, porque agrupar prejudica a leitura direta de "valor → significado".
