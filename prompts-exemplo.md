...
- [Templates de Prompts para Utilizar em Casos Reais no SEI](#templates-de-prompts-para-utilizar-em-casos-reais-no-sei)
  ...
  - [Revisão Técnica e de Segurança](#revisão-técnica-e-de-segurança)
    - [Revisar alterações](#revisar-alterações)
    - [Revisar um módulo ou um arquivo](#revisar-um-módulo-ou-um-arquivo)
    - [Revisar assuntos específicos](#revisar-assuntos-específicos)
...
|---|---|
| Uma branch antes do merge | `a branch <nome da branch>, comparada com <branch ou commit de referência>` |
| Um commit | `o commit <identificador do commit>` |
| Mais de um commit | `os commits <identificador 1> e <identificador 2>`, etc... |

**Prompt:**

```text
Use a skill `sei-revisao-tecnica` para revisar <o que revisar>.
```
...
As branches e os identificadores dos exemplos abaixo existem neste repositório. Troque pelos da sua mudança antes de enviar.

**Exemplo preenchido, avaliar o diff da branch antes do merge:**

```text
Use a skill `sei-revisao-tecnica` para revisar a branch origin/pet_v4.6.6, comparada com origin/master.
```

**Exemplo preenchido, um commit:**

```text
Use a skill `sei-revisao-tecnica` para revisar o commit 8bf4ab729d.
```

**Exemplo preenchido, mais de um commit:**

```text
Use a skill `sei-revisao-tecnica` para revisar os commits 29e8ee1308 e 19b852d280.
```

**Exemplo preenchido, com o contexto opcional no final:**
...
#### Revisar um módulo ou um arquivo

Use para levantar o estado atual do código que já está no repositório, incluindo o passivo antigo. Em um módulo, a revisão cobre todas as dimensões técnicas e também os scripts SEI e SIP de instalação e atualização mapeados para ele. Fonte: `sei-revisao-tecnica/SKILL.md`, seção "Escopo de modulo completo".

**Prompt para um módulo:**

```text
Use a skill `sei-revisao-tecnica` para fazer uma revisão técnica completa do módulo: <nome do módulo>
```

**Exemplo preenchido, um módulo inteiro:**

```text
Use a skill `sei-revisao-tecnica` para fazer uma revisão técnica completa do módulo: Peticionamento
```

**Prompt para um arquivo:**

```text
Use a skill `sei-revisao-tecnica` para fazer uma revisão técnica completa do arquivo: <caminho do arquivo>
```

**Exemplo preenchido, um arquivo:**

```text
Use a skill `sei-revisao-tecnica` para fazer uma revisão técnica completa do arquivo: fontes/sei/src/main/php/sei/web/modulos/litigioso/rn/MdLitLancamentoRN.php.
```

#### Revisar assuntos específicos

Use quando quiser limitar a revisão a um ou mais assuntos. A tabela traduz o assunto para o nome exato da dimensão usada pela skill. Escreva o nome como está na coluna da direita. Fonte: `sei-revisao-tecnica/SKILL.md`, seção "Dimensões técnicas".
...
```text
Use a skill `sei-revisao-tecnica` para revisar somente estas dimensões: <dimensões escolhidas>.

Alvo: <caminho do módulo ou arquivo>.
```

**Exemplo preenchido, permissões e transações:**

```text
Use a skill `sei-revisao-tecnica` para revisar somente estas dimensões: Permissoes e Transacao.

Alvo: fontes/sei/src/main/php/sei/web/modulos/peticionamento/rn/MdPetIntEmailNotificacaoRN.php.
```

**Exemplo preenchido, somente segurança:**

```text
Use a skill `sei-revisao-tecnica` para revisar somente estas dimensões: Seguranca.
...
Quando o conteúdo estiver preparado, antes de finalizar, execute uma revisão e pelo menos uma verificação independente de **correção gramatical e ortográfica em Português brasileiro** conforme as 'Regras de Escrita' do AGENTS.md.
...
```text
Use a skill `sei-revisao-tecnica` para fazer uma revisão técnica completa do módulo: ABC
